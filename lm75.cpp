...
nBlock_LM75::nBlock_LM75(PinName pinSDA, PinName pinSCL, int address, 
	...
	uint8_t config_value = __LM75_CONF_FAULTQUEUE_4 | ((comparatorPolarity & 0b00000001) << __LM75_CONF_POLARITY_Pos);
	i2cWrite(__LM75_CONFIG, config_value, 1);

	// Configures low temperature threshold for comparator
	uint16_t t_hyst = ((uint16_t)round(alarmMin * 2.0)) << 7;
	i2cWrite(__LM75_ALARM_LOW, t_hyst, 2);
	
	// Configures high temperature threshold for comparator
	uint16_t t_os = ((uint16_t)round(alarmMax * 2.0)) << 7;
	i2cWrite(__LM75_ALARM_HIGH, t_os, 2);
...
void nBlock_LM75::endFrame(void) {
	...
	if (_flagReadRequested) {
		...
		float temperature_shifted = i2cRead(__LM75_TEMPERATURE, 2) * 1.0; // temperature is *signed* int
...
void nBlock_LM75::i2cWrite(uint8_t regAddress, uint16_t value, uint32_t num_bytes) {
...
uint16_t nBlock_LM75::i2cRead(uint8_t regAddress, uint32_t num_bytes) {
